...
def run_classification_tendance_semestrielle():
    print("=" * 70)
    print("RANDOM FOREST - PREDICTION SEMESTRIELLE (AVEC CV)")
    print("=" * 70)
    ...
    zones_valides = [z for z in zones if len(etat_zones[z]["accidents"]) >= 6]
    ...
    output_file = DATA_PATH / "predictions_tendance_semestrielle_2023_2030.csv"
...
def test_accuracy_semestriel():
    try:
        df_pred = pd.read_csv(DATA_PATH / "predictions_tendance_semestrielle_2023_2030.csv")
    ...
    print("\n" + "=" * 60)
    print("ACCURACY - MODELE SEMESTRIEL")
    print("=" * 60)
...
df_futur, best_params = run_classification_tendance_semestrielle()
print(f"\nMeilleurs parametres: {best_params}")
test_accuracy_semestriel()
#pip install pandas numpy scikit-learn pyproj scipy matplotlib seaborn folium branca tqdm xgboost joblib