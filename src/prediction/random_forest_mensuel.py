...
def run_classification_tendance_mensuelle():
    ...
    # Encodage cyclique du mois (décembre et janvier sont "proches", pas 1 et 12)
    ...
    data["moyenne_12"] = data.groupby("zone")["nb_accidents"].transform(
        lambda x: x.shift(1).rolling(12, min_periods=6).mean()
    )
    data["evolution_recente"] = np.where(
        data["moyenne_12"] > 0, ((data["moyenne_3"] - data["moyenne_12"]) / data["moyenne_12"]) * 100, 0
    )
    ...
    FEATURES = ["annee", "mois_sin", "mois_cos", "densite_moyenne", "densite_max", "nombre_navires",
                "nb_accidents_lag_1", "nb_accidents_lag_2", "nb_accidents_lag_3", "nb_accidents_lag_12",
                "moyenne_3", "moyenne_12", "evolution_recente"]
 
    data_modele = data.dropna(subset=FEATURES + ["categorie"]).copy()
    date_coupure = pd.Timestamp("2023-01-01")
    train = data_modele[data_modele["mois"] < date_coupure]
    ...
    dernier_mois_connu = mois_max  # dernier mois réellement disponible dans les données
    ...
    zones_valides = [z for z in zones if len(etat_zones[z]["accidents"]) >= 12]
    print(f"{len(zones_valides)}/{len(zones)} zones avec assez d'historique (>= 12 mois)")
    ...
    for mois in mois_futurs:
        ...
        for zone in zones_valides:
            ...
            lag1, lag2, lag3, lag12 = accidents_list[-1], accidents_list[-2], accidents_list[-3], accidents_list[-12]
            moyenne_3 = np.mean(accidents_list[-3:])
            moyenne_12 = np.mean(accidents_list[-12:])
            evolution_recente = ((moyenne_3 - moyenne_12) / moyenne_12 * 100) if moyenne_12 > 0 else 0
 
            lignes_features.append([
                annee_courante, mois_sin, mois_cos, s["densite"][-1], s["densite_max"][-1], s["navires"][-1],
                lag1, lag2, lag3, lag12, moyenne_3, moyenne_12, evolution_recente
            ])
...
def test_accuracy_mensuel():
    ...
    ref_zone = accidents_par_zone[accidents_par_zone["annee"] <= 2022].groupby(["zone", "mois"])["nb_accidents"].mean().reset_index()
...
run_classification_tendance_mensuelle()
